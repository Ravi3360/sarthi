...
1. **SDK**: `@react-native-firebase/*` (native modules: `app`, `auth`, `firestore`, `storage`, `functions`). **Revised from the originally approved Firebase JS SDK** — during plan-writing it was confirmed that the JS SDK's Firestore offline persistence (`persistentLocalCache`) is IndexedDB-based and does not work on React Native (only in browsers); real Firestore offline caching on RN only exists in the native SDK. Since offline support (Decision 6) matters more than keeping plain Expo Go, this trade is accepted:
...
2. **Auth**: Firebase Anonymous Authentication under a mock OTP gate.
   ...
   - On success: sign in anonymously, then call the `linkWorkerAuth` callable Cloud Function (see Decision 7 correction below) with the fresh anon uid and `mobile`. The function looks up `phoneIndex/{mobile}` for an existing `uid` under Admin privileges: if found, it treats the **looked-up** uid as the worker id (not `auth.currentUser.uid`) and array-unions the new anon uid into that worker doc's `linkedAuthUids`, returning `{ uid: resolvedUid, isNew: false }`; if not found, it creates `phoneIndex/{mobile} = { uid: freshUid }` and returns `{ uid: freshUid, isNew: true }`, and the client then creates `workers/{freshUid}` (with `linkedAuthUids: [freshUid]`) directly — that create is allowed by client-side rules since `request.auth.uid == uid` at that point.
...
7. **Security rules** (core logic; test-mode-grade, not production-hardened):
   ...
   **Correction (found during final whole-branch review, after this design's initial approval, addressed in plan Task 11):** the original design's comment above — `phoneIndex` "written only via trusted client logic" — was self-contradictory with `allow write: if false`, and the `workers/{uid}` update rule as written can never admit the `linkedAuthUids` arrayUnion a client would need to perform on its own uid before that uid is itself a member of the array. Rather than loosen either rule (which would weaken the intended access model), both trusted writes — creating `phoneIndex/{mobile}`, and array-unioning a fresh anon uid into an existing worker's `linkedAuthUids` — are performed server-side by a callable Cloud Function (`functions/src/index.ts`, `linkWorkerAuth`), which runs under the Admin SDK and bypasses Firestore rules entirely. The rule *values* above are unchanged; only the stale comment was wrong. This requires the Firebase project to be on the Blaze (pay-as-you-go) plan — Cloud Functions are not available on the free Spark plan (see Decision 8).

   A second, related correction from the same review: resolving a worker at auth-boot time (`context/AuthContext.tsx`'s `onAuthStateChanged`) by a direct `workers/{uid}` doc-id `get()` on `auth.currentUser.uid` fails for a returning worker after a real re-login, because the persisted session's uid is only a member of `linkedAuthUids`, not the doc id — every subsequent cold boot would silently create a fresh empty worker. Fixed by resolving via an `array-contains` query on `linkedAuthUids` instead (`services/workers.ts`'s `findWorkerByAuthUid`), which matches uniformly whether the current auth uid is the original doc id or a later-linked one. No rule change is needed for this either: Firestore permits a `list`/query whose filter is provably a subset of what the per-document rule allows, and an equality/`array-contains` filter against `request.auth.uid` satisfies that.

8. **Firebase project**: does not exist yet. Implementation plan must include console setup steps: create project, register **native** iOS + Android apps under app id `com.saathi.app` (downloading `GoogleService-Info.plist` / `google-services.json`, referenced from `app.json`), enable Firestore + Anonymous Auth + Storage. No web app registration needed (native SDK, not the JS SDK). **Correction (Task 11):** the project must also be upgraded to the Blaze (pay-as-you-go) billing plan to deploy the `linkWorkerAuth` Cloud Function — Cloud Functions depend on Cloud Build/Artifact Registry, unavailable on the free Spark plan. A real, if typically small (free-tier-covered at this app's expected volume), cost implication not present in the rest of this design.
...
- `lib/firebase.ts` (new) — thin re-export of the RNFirebase default instances (`@react-native-firebase/app`, `auth`, `firestore`, `storage`, `functions`); native config comes from the platform `GoogleService-Info.plist`/`google-services.json` files (auto-picked-up by the native SDK), not JS-side env vars.
- `services/auth.ts` — rewritten internals per Decision 2, using `auth().signInAnonymously()` / `auth().signOut()` / `auth().onAuthStateChanged()`; same exported function signatures as today. **Correction (Task 11):** the `phoneIndex`/`linkedAuthUids` trusted writes are delegated to the `linkWorkerAuth` callable Cloud Function (`httpsCallable(functions, 'linkWorkerAuth')`) rather than written directly by the client — see Decision 7 correction.
- `services/workers.ts`, `documents.ts`, `income.ts`, `workHistory.ts`, `skills.ts` — internals swapped from AsyncStorage calls to Firestore (`firestore().collection(...).doc(...).get()/.set()/.update()`, subcollection CRUD); same exported function signatures, so **no changes required in `app/*` screens or `context/*`**. `services/workers.ts` additionally exports `findWorkerByAuthUid(authUid)` (Task 11 correction), an `array-contains` query on `linkedAuthUids` used by `context/AuthContext.tsx` to resolve the worker at auth-boot time — see Decision 7 correction.
- `functions/` (new, Task 11 correction) — a separate small Node/TypeScript Cloud Functions project (its own `package.json`/`tsconfig.json`, Admin SDK, Node 22 runtime — independent of the app's own Node 24 dev-tooling version), deployed via `firebase deploy --only functions`. Contains the `linkWorkerAuth` callable that performs the two trusted writes described in Decision 7's correction.
...
- The `linkWorkerAuth` Cloud Function (`functions/src/index.ts`, added in Task 11) and `findWorkerByAuthUid` (`services/workers.ts`, added in Task 11) are deleted along with it — Phone Auth's stable per-number uid makes both trusted-linking and array-contains boot resolution unnecessary; boot resolution reverts to a plain `getWorker(auth.currentUser.uid)` doc-id lookup.
